...
describe('/notifications/get Testing', () => {
  ...
  test('Tagged, Reacted, and Added (Channel)', () => {
    ...
    testMessageReact(user1.token, user2message.messageId, 1);

    expect(testNotificationsGet(user2.token)).toStrictEqual({
      notifications: [{
        channelId: channel.channelId,
        dmId: -1,
        notificationMessage: '@first1last1 reacted to your message in channel'
      }, {
        channelId: channel.channelId,
        dmId: -1,
  ...
  test('Tagged, Reacted, and Added (Dm)', () => {
    ...
    expect(testNotificationsGet(user2.token)).toStrictEqual({
      notifications: [{
        channelId: -1,
        dmId: dm.dmId,
        notificationMessage: '@first1last1 reacted to your message in first1last1, first2last2'
      }, {
        channelId: -1,
        dmId: dm.dmId,
